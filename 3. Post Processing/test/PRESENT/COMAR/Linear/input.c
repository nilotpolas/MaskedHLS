void sbox(_Bool x0_0, _Bool x1_0, _Bool x2_0, _Bool x3_0, _Bool x0_1, _Bool x1_1, _Bool x2_1, _Bool x3_1, _Bool r0, _Bool r1, _Bool r0_1, _Bool r1_1, _Bool r2_1, _Bool r3_1, _Bool *Y0_0, _Bool *Y1_0, _Bool *Y2_0, _Bool *Y3_0, _Bool *Y0_1, _Bool *Y1_1, _Bool *Y2_1, _Bool *Y3_1)
{
  ...
  Q2_0 = T0_0 ^ reg(reg(reg(L2_0)));
  L4_0 = T0_0 ^ T2_0;
  Q7_0 = T0_0 ^ reg(reg(reg(L5_0)));
  Q6_0 = L4_0 ^ reg(reg(reg(L3_0)));
  Q2_1 = T0_1 ^ L2_1;
  L4_1 = T0_1 ^ T2_1;
  Q7_1 = T0_1 ^ L5_1;
  Q6_1 = L4_1 ^ L3_1;
  ...
  p2_comar2 = reg(m0_comar2) & reg(reg(reg(reg(m1_comar2))));
  p3_comar2 = reg(m3_comar2) & reg(m2_comar2);
  p1_comar2 = reg(m0_comar2) & reg(reg(reg(reg(m2_comar2))));
  p4_comar2 = reg(m3_comar2) & reg(m1_comar2);
  i0_comar2 = p1_comar2 ^ reg(reg(reg(reg(r0_1_inp))));
  i1_comar2 = p2_comar2 ^ reg(reg(reg(reg(r1_1_inp))));
  i2_comar2 = p3_comar2 ^ reg(r2_1_inp);
  i3_comar2 = p4_comar2 ^ reg(r3_1_inp);
  i1xori2_comar2 = reg(i1_comar2) ^ reg(reg(reg(reg(i2_comar2))));
  i0xori3_comar2 = reg(i0_comar2) ^ reg(reg(reg(reg(i3_comar2))));
  ...
  p2_comar3 = reg(m0_comar3) & reg(reg(reg(reg(m1_comar3))));
  p3_comar3 = reg(reg(reg(reg(m3_comar3)))) & reg(m2_comar3);
  p1_comar3 = reg(m0_comar3) & reg(m2_comar3);
  p4_comar3 = reg(m3_comar3) & reg(m1_comar3);
  i0_comar3 = p1_comar3 ^ reg(reg(reg(reg(r0_1_inp))));
  i1_comar3 = p2_comar3 ^ reg(reg(reg(reg(r1_1_inp))));
  i2_comar3 = p3_comar3 ^ reg(reg(reg(reg(r2_1_inp))));
  i3_comar3 = p4_comar3 ^ reg(r3_1_inp);
  i1xori2_comar3 = reg(i1_comar3) ^ reg(i2_comar3);
  i0xori3_comar3 = reg(i0_comar3) ^ reg(reg(reg(reg(i3_comar3))));
  ...
  L11_0 = T1_0 ^ reg(reg(reg(reg(reg(reg(L10_0))))));
  L7_1 = T0_1 ^ T1_1;
  L11_1 = T1_1 ^ L10_1;
  Y0_01 = L7_0 ^ reg(reg(reg(T2_0)));
  Y1_01 = reg(reg(reg(reg(reg(reg(L8_0)))))) ^ T3_0;
  Y0_11 = L7_1 ^ T2_1;
  Y1_11 = L8_1 ^ T3_1;
  *Y0_0 = reg(reg(reg(reg(reg(reg(reg(x3_0_inp)))))) ^ Y0_01);
  *Y1_0 = reg(L7_0 ^ Y1_01);
  *Y2_0 = reg(L11_0 ^ reg(reg(reg(T2_0))));
  *Y3_0 = reg(reg(reg(reg(T2_0 ^ reg(reg(reg(L5_0)))))));
  *Y0_1 = reg(reg(reg(reg(reg(reg(reg(x3_1_inp ^ Y0_11)))))));
  *Y1_1 = reg(reg(reg(reg(reg(reg(reg(L7_1 ^ Y1_11)))))));
  *Y2_1 = reg(reg(reg(reg(reg(reg(reg(L11_1 ^ T2_1)))))));
  *Y3_1 = reg(reg(reg(reg(reg(reg(reg(T2_1 ^ L5_1)))))));
}
